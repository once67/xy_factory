...
with open(os.path.join(base_dir, 'model_1d.pickle'), 'rb') as f:
    temp_1 = pickle.load(f)
    model_1d = Model_ND(temp_1, 1)

with open(os.path.join(base_dir, 'model_2d.pickle'), 'rb') as f:
    temp_2 = pickle.load(f)
    model_2d = Model_ND(temp_2, 2)

# main.py 末尾添加：
def generate_by_text(text, model_1d, model_2d):
